...
class MesOffresController extends AbstractController
{
    ...
    public function deleteOffre($id)
    {
        ...
        for ($i = 0; $i < count($contents["rows"]); $i++)
            if ($contents["rows"][$i]["doc"]["_id"] == $id) {
                ...
                for ($k = 0; $k < count($contents["rows"][$i]["doc"]["postulants"]); $k++) {
                    ...
                    $add_notif_response = $client->request('POST', 'https://d10080de.ngrok.io/notifications', [
                        "headers" => [
        ...
        $response_delete = $client->request('DELETE', 'https://d10080de.ngrok.io/annonces/' . $id . "/", [
            'query' => [
                'rev' => $rev
            ]
        ]);
        ...
        $response_all_annonces = $client->request('GET', 'https://d10080de.ngrok.io/annonces/_all_docs?include_docs=true');
